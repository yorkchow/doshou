...
<ul class="nav nav-tabs">
        ...
        <c:if test="${op eq '新增'}">
            <c:forEach items="${types}" var="t">
                <li ${t eq type ? 'class="active"' : ''}>
                    <a href="${ctx}/admin/sys/auth/${t}/create?BackURL=<doshou:backURL/>">
    ...
    <c:if test="${not empty m.id}">
        <li ${op eq '查看' ? 'class="active"' : ''}>
            <a href="${ctx}/admin/sys/auth/${m.id}?BackURL=<doshou:backURL/>">
        ...
        <shiro:hasPermission name="sys:auth:update">
            <li ${op eq '修改' ? 'class="active"' : ''}>
                <a href="${ctx}/admin/sys/auth/${m.id}/update?BackURL=<doshou:backURL/>">
                    <i class="icon-edit"></i>
                    修改
                </a>
            </li>
        ...
        <shiro:hasPermission name="sys:auth:delete">
            <li ${op eq '删除' ? 'class="active"' : ''}>
                <a href="${ctx}/admin/sys/auth/${m.id}/delete?BackURL=<doshou:backURL/>">
                    ...
                    <div class="list">
                        <ul>
                            <c:forEach items="${roles}" var="r">
                                <c:if test="${not dsfn:in(m.roleIds, r.id)}">
                ...
                <div class="right" id="roleIds_msg" data-prompt-position="topLeft">
                    ...
                    <div class="list">
                        <ul data-input-id="roleIds">
                            <c:forEach items="${roles}" var="r">
                                <c:if test="${dsfn:in(m.roleIds, r.id)}">
                                    <li class="ui-state-default" data-value="${r.id}" title="${r.description}">
                                        ${r.name}[${r.role}]
                                    </li>
                                </c:if>
